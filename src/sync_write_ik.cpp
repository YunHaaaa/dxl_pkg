...
double t = 0;
int T = 5;
uint32_t dxl_goal_position_cos = 0;
...
double r2t(double theta) {
  return theta * 2047 / M_PI;
}

struct IK(double x, double y) {
  ...
  double L2 = pow(target.x, 2) + pow(target.y, 2);
  double beta = acos(((pow(l1, 2) + pow(l2, 2))-L2)/(2*l1*l2));

  double theta2 = M_PI - beta;
  double theta1 = atan2(y,x) - asin(l2/sqrt(L2)*sin(beta));

  //double dxl_real_position[2];
  EP_goal.x = r2t(M_PI - beta);
  EP_goal.y = r2t(atan2(y,x) - asin(l2/sqrt(L2)*sin(beta)));

//  //int dxl_goal_position[2] = { 1024, 3072 };
//  dxl_goal_position[2] = {theta1, theta2};
//  dxl_goal_position_cos = (((dxl_goal_position[idx]-dxl_real_position[idx]) / 2) * (1-cos(2 * 3.14 * t / T))) + dxl_real_position[idx];
  target.x = E.x + (Ep_goal.x - E.x) * 0.5 (1-cos(M_PI * t / T));
...
*/
        // motor1
    param_goal_position[0] = DXL_LOBYTE(DXL_LOWORD());
    param_goal_position[1] = DXL_HIBYTE(DXL_LOWORD());

    // motor2
    param_goal_position[2] = DXL_LOBYTE(DXL_HIWORD());
    param_goal_position[3] = DXL_HIBYTE(DXL_HIWORD());
